...
<script lang="ts" setup>
  ...
  const winnersList = ref<string[]>([]) // 获奖人员数组，用于标签式输入
  const newWinnerName = ref('') // 新的获奖人员姓名输入框
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      // 处理日期格式（确保为YYYY-MM-dd，避免重复添加-01）
      ...
      awardForm.year = Number(awardForm.awardDate?.slice(0, 4)) // 提取YYYY部分
