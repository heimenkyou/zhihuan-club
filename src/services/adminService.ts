  ...
  try {
    ...
    if (params.keyword) {
      ...
      filteredAwards = rawAwards.filter((award: AwardItem) =>
        ...
        (Array.isArray(award.winners) ? award.winners.some(winner => winner?.toLowerCase().includes(keyword)) : typeof award.winners === 'string' ? award.winners.toLowerCase().includes(keyword) : false) ||
...
export const getUnreferencedMedia = async (): Promise<MediaResource[]> => {
  try {
    ...
    else if (response.data.mediaResources && Array.isArray(response.data.mediaResources)) {
    ...
    else if (typeof response.data === 'object' && response.data.id && response.data.url) {
...
// 添加缺失的项目相关API函数
// 创建项目
export const createProject = async (params: Omit<Project, 'id'>): Promise<Project> => {
  try {
...
// 更新项目
export const updateProject = async (id: number, params: Omit<Project, 'id'>): Promise<Project> => {
  try {
...
// 添加getProjects函数定义
export const getProjects = async (params?: { current?: number; size?: number; keyword?: string }): Promise<ProjectPageData<Project>> => {
  try {
    ...
    if (typeof response.data === 'object') {
      ...
      else if ((response.data.code === '0' || response.data.code === '3') && response.data.data) {
        return response.data.data;
      }
      // 支持直接返回分页数据
