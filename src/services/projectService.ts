import api from './api';
import type { AwardItem } from './adminService';
...
export interface ProjectDetail {
  ...
  mediaResources?: Array<{id: number, url: string, type: string, title?: string, description?: string, caption?: string}>;
  // 添加 awards 字段（后端使用的字段名）
  awards?: Array<{date?: string, title?: string, description?: string, type?: string}>;
...
function processProjectData(data: any): ProjectDetail | null {
  ...
  // 处理技术栈字段兼容
  if (projectData.techStackDetail && Array.isArray(projectData.techStackDetail)) {
    // 如果techStackDetail是对象数组，提取name字段
    if (projectData.techStackDetail.length > 0 && typeof projectData.techStackDetail[0] === 'object') {
      projectData.techStackDetail = projectData.techStackDetail.map((tech: any) => tech.name);
    }
  }
  // 处理图片URL中的空格和反引号
