<template>
  <AdminPage title="管理员管理">
    ...
    <p class="total-count">共 {{ total }} 名管理员</p>
  </AdminPage>
...
const load = async () => {
	try {
		...
		total.value = Number(data?.total ?? admins.value.length);
	} catch (error) {
...
<style scoped>
:deep(.danger-item) { color: var(--el-color-danger); } .total-count { margin: 12px 0 0; color: #667085; font-size: 13px; text-align: right; } @media (max-width: 768px) { :deep(.admin-dialog) { width: calc(100% - 24px) !important; margin: 12px auto; } .total-count { text-align: center; } }
</style>
